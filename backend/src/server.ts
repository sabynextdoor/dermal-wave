...
app.get('/api/consultants', authenticate, async (req: Request, res: Response) => {
...
app.get('/api/dashboard', authenticate, async (req: Request, res: Response) => {
  ...
  const scoreData = scans.map((s: any, idx: number) => ({ 
    name: `Scan ${idx+1}`, 
    score: Math.max(0, 100 - s.severity) 
  }));

  const avgSeverity = scans.length > 0 
    ? Math.round(scans.reduce((acc: number, s: any) => acc + s.severity, 0) / scans.length)
    : 0;
